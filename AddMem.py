...
cn = pymysql.connect(host="localhost", user="root", password=mypass, database=mydatabase)
cr = cn.cursor()
# cr = sql connection
...
def addmem():
    ...
    pr = "insert into mem values('"+p1+"','"+p2+"','"+p3+"','"+p5+"')"
    ...
    if p==None:
        try:
            cr.execute(pr)
            cn.commit()
            messagebox.showinfo('Message',"Member Added Successfully")
        except:
